...
describe("[Challenge] Puppet v2", function () {
  ...
  it("Execution", async function () {
    ...
    let amount = await lendingPool.calculateDepositOfWETHRequired(
      token.balanceOf(lendingPool.address)
    );
    await weth.connect(player).deposit({ value: amount });
    await weth.connect(player).approve(lendingPool.address, amount);
    await lendingPool
      .connect(player)
